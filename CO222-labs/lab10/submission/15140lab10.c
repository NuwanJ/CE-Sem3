/******************************************************************************************************
	Lab10 : Jaliyagoda A.J.N.M. (E/15/140)

	Note:  
		1. '.' and ',' marks are removed from the end of the words, because they aren't related to words.
		2. Have a doubt on removing all punctuation marks; no any sample program provided. 
...
void removeDuplicates(Node * head, char * str);
...
int main(int argc, char **argv){
	...
	while (fscanf(fp, "%s", str)!=EOF){
		...
		//if(str[len-1] == '.' || str[len-1] == ',') str[len-1] = '\0';
	...
	*/
	...
	while(current != NULL){
		//printf("%s\n", current->word);
		removeDuplicates(current, current->word);
	...
	if(head->next != NULL){
		...
		while(current->next != NULL){
			if(strcmp(current->word, str)==0){		
				// need to remove the current node from the list
